...
A live JSON build must contain {sourceFile:"assets/<bundle>/<scene>.fire", tree:{...}, dirty:true|false|"unknown"}.`;
...
try {
  if (cmd === 'build') {
    ...
    console.log(JSON.stringify({ ok: true, outDir, shards, parserVersion: manifest.parserVersion, engineProfile: manifest.engineProfile, builtAt: manifest.builtAt }));
  } else if (cmd === 'query') {
    ...
    console.log(JSON.stringify({
      ...
      comps: graph.comps.length,
      refs: graph.refs.length,
      serializedNodes: graph.serializedNodes,
      indexedNodes: graph.indexedNodes,
      omittedNodes: graph.omittedNodes,
      serializedComponents: graph.serializedComponents,
      indexedComponents: graph.indexedComponents,
      omittedComponents: graph.omittedComponents,
      prefabOpaque: !!graph.prefabOpaque,
      source: graph.source,
      dirty: graph.dirty,
      engineProfile: graph.engineProfile,
      shard,
    }));
  } else {
    usage(2);
