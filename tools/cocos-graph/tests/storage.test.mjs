...
const fixture = readFileSync(join(testDir, 'fixtures', 'mini.scene.json'), 'utf8');
...
describe('transactional graph storage', () => {
  ...
  it('serializes two real build processes in the same namespace', async () => {
    ...
    writeFileSync(join(bundleDir, 'a.fire'), fixture);
    const cli = join(testDir, '..', 'bin', 'cocos-graph.mjs');
    try {
