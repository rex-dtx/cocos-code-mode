...
describe('decodeUuid', () => {
  ...
  it('decodes Creator 2.4 script class IDs', () => {
    assert.equal(decodeUuid('39dc6KlvOBMs7kOVtL9JMKP'), '39dc62a5-bce0-4cb3-b90e-56d2fd24c28f');
    assert.equal(decodeUuid('eea96cbTy1OLb5mD7LrZfNP'), 'eea9671b-4f2d-4e2d-be66-0fb2eb65f34f');
    assert.equal(decodeUuid('521bfkh87VEXq6qNKcuG2T+'), '521bf921-f3b5-445e-aeaa-34a72e1b64fe');
  });
...
describe('parseEntries schema v4', () => {
  ...
  it('indexes stable 2.4 identities and reports opaque records', () => {
    const scene = parseSceneText(readFileSync(join(fixtures, 'creator-2x.fire.json'), 'utf8'), { file: 'assets/test/scene.fire' });
    assert.deepEqual({
      serializedNodes: scene.serializedNodes,
      indexedNodes: scene.indexedNodes,
      omittedNodes: scene.omittedNodes,
      serializedComponents: scene.serializedComponents,
      indexedComponents: scene.indexedComponents,
      omittedComponents: scene.omittedComponents,
      prefabOpaque: scene.prefabOpaque,
    }, {
      serializedNodes: 3,
      indexedNodes: 2,
      omittedNodes: 1,
      serializedComponents: 2,
      indexedComponents: 1,
      omittedComponents: 1,
      prefabOpaque: true,
    });
    assert.equal(scene.comps[0].script, '39dc62a5-bce0-4cb3-b90e-56d2fd24c28f');
    assert.equal(scene.refs[0].uuid, 'fc991dd7-0033-4b80-9d41-c8a86a702e59');

    const prefab = parseSceneText(readFileSync(join(fixtures, 'creator-2x.prefab.json'), 'utf8'), { file: 'assets/test/prefab.prefab' });
    assert.equal(prefab.indexedNodes, 1);
    assert.equal(prefab.omittedNodes, 1);
    assert.equal(prefab.nodes[0].uuid, 'prefab-root-file-id');
    assert.equal(prefab.prefabOpaque, true);
  });
