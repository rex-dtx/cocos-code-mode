...
function projectWithBundles() {
  ...
  writeFileSync(join(project, 'assets', 'bundle-a', 'a.fire'), fixture('mini.scene.json'));
  writeFileSync(join(project, 'assets', 'bundle-a', 'b.fire'), fixture('duplicate.scene.json'));
  writeFileSync(join(project, 'assets', 'bundle-b', 'only.fire'), fixture('mini.scene.json'));
  return project;
}
...
describe('builder schema v4 integrity', () => {
  it('builds versioned generations and rejects schema v3 with rebuild recovery', () => {
    ...
    try {
      ...
      assert.equal(graph.engineProfile, 'creator-2.4');
      ...
      const wrongProfile = { ...manifest, engineProfile: 'creator-3.x' };
      writeFileSync(join(outDir, '_manifest.json'), JSON.stringify(wrongProfile));
      assert.throws(() => loadShard(outDir, 'bundle-a'), /engineProfile .*creator-2\.4.*run: cocos-graph build/);
  ...
  it('build --bundle scope preserves and does not rewrite other shard generations', () => {
    ...
    try {
      ...
      writeFileSync(join(project, 'assets', 'bundle-a', 'a.fire'), fixture('duplicate.scene.json'));
  ...
  it('overlays one live scene while retaining unrelated disk files', () => {
    ...
    live.sourceFile = 'assets/bundle-a/a.fire';
    ...
    try {
      ...
      assert.ok(graph.nodes.some((node) => node.name === 'OtherPlayer' && node.file.endsWith('/b.fire') && node.source === 'disk'));
      assert.equal(graph.nodes.some((node) => node.file.endsWith('/a.fire') && node.source === 'disk'), false);
  ...
  it('keeps the last valid manifest and graph when parsing fails', () => {
    ...
    try {
      ...
      writeFileSync(join(project, 'assets', 'bundle-a', 'a.fire'), '{broken');
