...
export function loadShard(outDir, bundle) {
  ...
  if (manifest.engineProfile !== ENGINE_PROFILE) {
    throw new Error(`cocos-graph: engineProfile "${manifest.engineProfile ?? 'missing'}" is incompatible; "${ENGINE_PROFILE}" required (run: cocos-graph build ...)`);
  }
  ...
  if (graph.version !== PARSER_VERSION || graph.engineProfile !== ENGINE_PROFILE || graph.bundle !== bundle || record.sha256 !== actualHash) {
    throw new Error(`cocos-graph: shard ${bundle} is stale or unreadable (parserVersion "${PARSER_VERSION}", engineProfile "${ENGINE_PROFILE}" expected; run build)`);
  }
