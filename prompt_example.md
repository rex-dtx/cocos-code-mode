...
- If `CK_CODE_MODE=ready` or `.claude/cc-code-mode-cache.json` exists → **skip** `register_manual`/`list_tools`/`search_tools`/`tools_info`, go straight to `call_tool_chain("cc2x4.<tool>({ ... })")` (cc3x7 on 3.x branch, cc2x4 on 2.x).
