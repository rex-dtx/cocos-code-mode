...
- If `CK_CODE_MODE=ready` or `.claude/cc-remoter-cache.json` exists → **skip** `register_manual`/`list_tools`/`search_tools`/`tools_info`, go straight to `call_tool_chain("cc_remoter_2x.<tool>({ ... })")` — manual `cc-remoter-2x` (short `ccr-2x`->`ccr_2x`), legacy `cc2x4`/`cc-remoter-v2x4` van dung duoc (cc3x7 on 3.x branch).
