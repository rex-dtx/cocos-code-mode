...
async function discoverPort() {
    ...
    throw new Error('Cannot discover UTCP port: is the editor running cocos-pilot-3x?');
}
...
async function main() {
    ...
    const url = `${base}/tools/inspectorGetDefinition?target=CommonTypes`;
    const durs = [], sizes = [];
    ...
    durs.sort((a, b) => a - b);
    sizes.sort((a, b) => a - b);
    console.log(`\nSmoke ${REPEATS}x GET /tools/inspectorGetDefinition?target=CommonTypes`);
