...
(async () => {
  ...
  await test('E components', 'E5 getComponents', S(CLEAN + 'const node=new cc.Node(M);sc.addChild(node);node.addComponent(cc.Sprite);node.addComponent(cc.Label);const n=node.getComponents(cc.Component).length;node.destroy();return n;'), 'ok', r => r >= 2);
