...
describe('live: Sprite interactive asset workflow', { concurrency: false }, () => {
  ...
  it('creates a Canvas-hosted SpriteFrame node and mutates its public properties', async (t) => {
    ...
    const created = await postTool('createSprite', {
      name: '__ccb3x_sprite_interactive__',
    ...
    try {
      ...
      const transformed = await postTool('nodeBatchSet', {
        entries: [{
          reference: node,
          propertyPaths: ['position', 'scale'],
          values: [{ x: 140, y: -60, z: 0 }, { x: 1.25, y: 0.8, z: 1 }],
        }],
      });
      assert.equal(transformed.ok, true, JSON.stringify(transformed.body));

      const tinted = await postTool('inspectorSet', {
        target: 'instance',
        reference: sprite,
        propertyPath: 'color',
        value: { r: 40, g: 180, b: 240, a: 255 },
      });
      assert.equal(tinted.ok, true, JSON.stringify(tinted.body));

      const resized = await postTool('inspectorSet', {
        target: 'instance',
        reference: uiTransform,
      ...
      const readBack = await postTool('sceneBatchGet', {
        entries: [{ target: 'instance', reference: node, fields: ['position', 'scale'] }],
      });
