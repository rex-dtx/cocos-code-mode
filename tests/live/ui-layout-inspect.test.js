...
const { postTool, postExpectedErrorTool, getJson, repeatTestcase, healthCheck, getCanvasReference } = require('../helpers/utcp-client');
  ...
  it('uses Canvas-hosted UI nodes and preserves own geometry over HTTP', async t => {
    ...
    await repeatTestcase('UI-LAYOUT-I01', async () => {
      ...
      try {
        ...
        for (const [key, expected] of Object.entries({ x: 640, y: 365, width: 40, height: 20 })) assert.ok(Math.abs(node.worldRect[key] - expected) < 1e-5, `${key}: ${node.worldRect[key]} != ${expected}`);
