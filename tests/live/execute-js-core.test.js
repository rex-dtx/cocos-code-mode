...
describe('live: executeJavascript — core capabilities (A-M)', () => {
  ...
  const CLEAN = 'const sc=cc.director.getScene();const host=sc.getChildByName("Canvas");const M="__suite_tmp__";if(!host)return {skip:true};for(const n of [...host.children])if(n.name===M){n.removeFromParent();n.destroy();}';
  ...
  describe('E components', () => {
    ...
    it('E6 active flag toggle', async (t) => { if (skip(t)) return; const r = await exec('scene', CLEAN + 'const node=new cc.Node(M);host.addChild(node);node.active=false;const a=node.active;node.active=true;const b=node.active;node.destroy();return [a,b];'); assert.equal(r.ok, true); assert.deepEqual(resVal(r.body), [false, true]); });
  });
