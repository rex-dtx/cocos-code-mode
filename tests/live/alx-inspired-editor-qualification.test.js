  ...
  it('resolves bounded reverse paths and aligns/distributes sibling UI nodes', async (t) => {
    ...
    await repeatTestcase('EDITOR-ERGONOMICS-01', async () => {
      ...
      try {
        ...
        const align = await postTool('uiLayoutAlign', { references: ids.slice(0, 2).map((id) => ({ id, type: 'cc.Node' })), operation: 'align', axis: 'horizontal', edge: 'left' });
        ...
        const distribute = await postTool('uiLayoutAlign', { references: ids.map((id) => ({ id, type: 'cc.Node' })), operation: 'distribute', axis: 'horizontal' });
