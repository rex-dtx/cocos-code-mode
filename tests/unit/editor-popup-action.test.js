...
function actionFixture() {
  const owner = { id: 'native:1234:0X100', title: 'Creator', visible: true, ownerVerified: true, classification: 'creator-main', signals: ['native-class:Chrome_WidgetWin_1'], actions: [] };
  const popup = { id: popupId, title: 'Warn', visible: true, ownerVerified: true, classification: 'dialog', parentId: owner.id, signals: ['native-dialog-class', 'native-owner'], actions: [{ id: 'native:1234:0XDEF', label: 'Confirm', enabled: true }] };
...
describe('editorPopupAction', () => {
  ...
  it('rejects a stale native popup before operator approval or activation', async () => {
    mockCreator();
  ...
  it('requires operator approval and refuses cancellation without dispatch', async () => {
    const { snapshot } = actionFixture();
    let dispatched = 0;
    const dependencies = { inspect: async () => snapshot, approve: async () => ({ buttonIndex: 0, cancelled: true, timedOut: false }), activate: async () => { dispatched++; return { activated: true, closed: true }; }, notify: () => ({ id: 'notice' }) };
    await assert.rejects(() => actOnEditorPopup({ operation: 'activate', popupId, popupTitle: 'Warn', actionId: 'native:1234:0XDEF', actionLabel: 'Confirm', confirm: true, authorization: 'user-explicit' }, dependencies), error => error.code === 'POPUP_ACTION_UNAUTHORIZED');
    assert.equal(dispatched, 0);
  });

  it('activates exact approved action and proves the original popup closed', async () => {
    const { snapshot, popup } = actionFixture();
    let reads = 0;
    let dispatched = 0;
    const dependencies = { inspect: async () => ++reads === 1 ? snapshot : { complete: true, windows: snapshot.windows.filter(window => window.id !== popup.id) }, approve: async () => ({ buttonIndex: 1, cancelled: false, timedOut: false }), activate: async (...identity) => { dispatched++; assert.deepEqual(identity, [popup.id, popup.actions[0].id, popup.title, popup.actions[0].label, popup.parentId, 'Creator', 'Chrome_WidgetWin_1']); return { activated: true, closed: true }; }, notify: () => ({ id: 'notice' }) };
  ...
  it('registers an approval-gated POST with strict inputs and bounded action outputs', () => {
