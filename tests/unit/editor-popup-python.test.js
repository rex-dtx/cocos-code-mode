...
it('activates only a fresh exact native button on a disposable dialog', { skip: process.platform !== 'win32' }, async () => {
  ...
  try {
    ...
    await assert.rejects(exec('python', [probe, String(fixture.pid), dialog.hwnd, button.hwnd, 'wrong title', button.label, owner, ownerTitle, ownerClass, JSON.stringify(dialog.content)], { timeout: 3000, windowsHide: true }));
    ...
    const { stdout } = await exec('python', [probe, String(fixture.pid), dialog.hwnd, button.hwnd, dialog.title, button.label, owner, ownerTitle, ownerClass, JSON.stringify(dialog.content)], { timeout: 3000, windowsHide: true });
