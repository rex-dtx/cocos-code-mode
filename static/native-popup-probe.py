...
if __name__ == '__main__':
    if len(sys.argv) not in (2, 9) or not sys.argv[1].isdigit():
        raise SystemExit('usage: native-popup-probe.py <pid> [popupHwnd actionHwnd title label ownerHwnd ownerTitle ownerClass]')
    pid = int(sys.argv[1])
    print(json.dumps(activate(pid, *sys.argv[2:]) if len(sys.argv) == 9 else inspect(pid), separators=(',', ':')))
