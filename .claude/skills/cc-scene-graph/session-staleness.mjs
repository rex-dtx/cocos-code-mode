...
import { ENGINE_PROFILE, PARSER_VERSION } from '../../../tools/cocos-graph/src/manifest.mjs';
...
function main() {
  ...
  if (manifest.engineProfile !== ENGINE_PROFILE) {
    note(`index engineProfile=${manifest.engineProfile ?? 'missing'} is incompatible; ${ENGINE_PROFILE} required — rebuild before querying`);
    return { banner, stale };
  }
  ...
  if (shard.dirty !== false) {
    note(`shard "${shard.name}" dirty=${shard.dirty ?? 'unknown'} — structure is advisory until sceneInfo confirms the live scene`);
  }
  if (shard.prefabOpaque === true) {
    note(`shard "${shard.name}" prefabOpaque=true — disk records omit nodes without stable identity; verify the target with sceneSnapshot`);
  }
  ...
  banner.push(stale
    ? 'stale signal above: re-check ccb2x.sceneInfo() and fall back to sceneSnapshot before any write'
    : 'dirty is unknown offline — call ccb2x.sceneInfo() before relying on structure for a write');
  return { banner, stale };
}
