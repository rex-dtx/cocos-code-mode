...
`cat .claude/cc-bridge-cache.json | jq '.manuals."cc-bridge-3x".toolDefs[] | .name'` hoac `echo $CK_CODE_MODE`.

## Quy tac goi tool

1. **Cache hit** (`CK_CODE_MODE=ready` hoac `.claude/cc-bridge-cache.json` co `tools[]`): goi thang
   `call_tool_chain("ccb3x.<tool>({ ... })")` (recommend) hoac `cc_bridge_3x.*` / `ccb_3x.*` (compat) — KHONG `register_manual`, KHONG `search_tools`/`list_tools`/`tools_info`.
