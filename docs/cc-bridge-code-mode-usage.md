...
Every HTTP tool interaction emits a concise `[cx3][api]` lifecycle event. When the server starts, it emits `LISTENING <- http://localhost:<port>/utcp`. With verbose logging enabled, each call is shown as `REQUEST -> METHOD /path tool=name`, followed by `RESPONSE <- STATUS tool=name duration=Nms` or `ERROR STATUS tool=name duration=Nms`; the same event includes bounded JSON details after `|`. By default, the extension is quiet: only warnings and errors are visible in the Creator console. Open **CC Bridge 3x > Configuration**, then enable **Verbose tool interaction logs** to show the full API lifecycle. The setting persists per project and also writes lifecycle events to `~/.utcp-debug/*.jsonl`. Other extension diagnostics use explicit groups such as `[cx3][config]`, `[cx3][scene]`, `[cx3][asset]`, `[cx3][ui]`, `[cx3][prompt]`, and `[cx3][control]`.
