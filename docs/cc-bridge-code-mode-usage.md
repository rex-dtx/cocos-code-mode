...
Every HTTP tool interaction emits a concise `[cx3][api][request-token]` lifecycle event. Requests appear as `REQUEST -> METHOD /path tool=name`; successful calls as `RESPONSE <- STATUS tool=name duration=Nms`; failures as `ERROR STATUS tool=name duration=Nms CODE: message`. The request token is the first eight characters of the full request ID, making request/response correlation visible without printing raw JSON. Full structured fields, request arguments, responses, error details, recovery guidance, and stacks remain in `~/.utcp-debug/*.jsonl` when verbose logging is enabled. By default, the extension is quiet: only warnings and errors are visible in the Creator console. Open **CC Bridge 3x > Configuration**, then enable **Verbose tool interaction logs** to show the full API lifecycle. The setting persists per project. Other extension diagnostics use explicit groups such as `[cx3][co…`
