...
**Install:** junction `<project>\packages\cc-bridge-2x` (legacy `cocos-code-mode` → `G:\_ws\_helpers\cc-code-mode-cst-2x`
...
```
success: cc-bridge-2x loaded
normal: [cc-bridge-2x] UTCP Server started on port 59142
```
...
`Editor.Profile.load('profile://project/cc-bridge-2x.json', {...})` chạy thẳng, không throw. Editor register sẵn type `project`. File đẻ ra ở `<project>/settings/cc-bridge-2x.json`.
...
Docs `main/console.md` chỉ có `log`/`warn`/`error`/`clearLog` — **ghi**, không đọc. Tool đã xoá khỏi code và khỏi `cc-bridge-2x.d.ts`. User xem Console panel trực tiếp. (Unresolved #1 phase 7 → resolved: không có.)

## `projectGetConfig` — đọc thẳng `settings/*.json`

`Editor.Profile.load` trả EventEmitter không serialize được (bẫy 4). Đọc file trực tiếp. Testbed có 5 file: `builder` `builder.panel` `cc-bridge-2x` `project` `services` (legacy `cocos-code-mode`).
...
| Typings | `typescript-defenition` (2 tool) | Sinh `.d.ts` từ property dump 3.x. 2.x thay bằng `cc-bridge-2x.d.ts` viết tay — **có chủ đích**, không phải nợ |
...
11. Config panel UI chưa port — vòng 1 server tự start, đọc port ở `settings/cc-bridge-2x.json` (legacy `cocos-code-mode.json`).
