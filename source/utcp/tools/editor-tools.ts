...
export class EditorTools {
    ...
    @utcpTool(
        'editorAsk',
        'Ask via nonmodal Agent Inbox choice buttons by default; never opens/focuses a panel unless openPanel:true. Inbox is no longer in the extension menu; use explicit openPanel:true when user interaction requires opening it. Await a choice or deadline (default 60s, max 5min). presentation:native explicitly opts into a modal Creator dialog; its timeout cannot dismiss the window. Default buttons OK/Cancel; cancelId defaults to the last button.',
    ...
    @utcpTool('editorState', 'Read bounded project path, current scene identity/readiness/dirty state, task counts, pending inbox metadata, and a detection-only popup summary. No scene tree, prompt values or popup detail records. Deadline default 1s, max 5s; unavailable fields are null and listed. busy.scene means scene not ready, not a global editor lock. Read-only and never focuses panels.', EditorStateInputSchema, EditorStateOutputSchema, 'GET', ['editor', 'state', 'task', 'inbox', 'popup'])
    ...
    @utcpTool('editorPopupAction', 'Remind the operator or activate one verified native Creator Button after an explicit caller decision for this exact popup and button. Requires exact popup/action IDs and labels, confirm:true, authorization:user-explicit; the service and bounded Python Win32 dispatcher independently re-read ownership, title, class, label, enabled state and prove exact popup closure. Never infer Save/Don\'t save/Confirm from title or order. No Agent Inbox wait, coordinate click, or autonomous choice. Unknown or stale controls fail closed.', EditorPopupActionInputSchema, EditorPopupActionOutputSchema, 'POST', ['editor', 'popup', 'dialog', 'operator', 'action'])
