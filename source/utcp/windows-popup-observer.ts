...
interface NativeWindowRow { hwnd: string; pid: number; visible: boolean; title: string; className: string; ownerHwnd: string | null; rootOwnerHwnd: string | null; bounds: PopupBounds | null; actions: NativeActionRow[] }
...
function normalizedRows(value: unknown, processId: number): NativeWindowRow[] {
    ...
    return (value as NativeProbeOutput).windows.slice(0, NATIVE_MAX_ITEMS).flatMap(row => {
        ...
        return [{ hwnd, pid: processId, visible: row.visible, title: boundedText(row.title, 256), className: boundedText(row.className, 256), ownerHwnd: boundedHwnd(row.ownerHwnd), rootOwnerHwnd: boundedHwnd(row.rootOwnerHwnd), bounds: boundedBounds(row.bounds), actions }];
    });
}
...
export async function activateWindowsPopupAction(popupId: string, actionId: string, popupTitle: string, actionLabel: string, ownerId: string, ownerTitle: string, ownerClass: string): Promise<{ activated: boolean, closed: boolean }> {
    ...
    const python = resolvePython();
    const args = [...python.prefix, probePath(), String(process.pid), popup[2].toUpperCase(), action[2].toUpperCase(), popupTitle, actionLabel, owner[2].toUpperCase(), ownerTitle, ownerClass];
