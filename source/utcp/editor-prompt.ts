...
function notify(event: string, ...args: string[]): void {
    ...
    catch (error) { console.warn('[cc-bridge-3x] Agent Inbox notification failed:', error); }
}
...
function startPrompt(request: EditorInteractionRequest): Promise<EditorPromptResult> {
    ...
    return new Promise((resolve, reject) => {
        ...
        pending = active;
        console.info(`[cc-bridge-3x] Agent request ${request.requestId} pending. Open CC Bridge 3x > Agent Inbox to respond before ${new Date(request.expiresAt).toISOString()}.`);
